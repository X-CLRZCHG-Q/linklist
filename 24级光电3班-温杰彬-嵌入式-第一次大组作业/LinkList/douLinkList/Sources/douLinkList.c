...
Node *initList()
{
	...
	head->prev=NULL;                       //首节点的prev域赋初值NULL
	head->next=NULL;                       //首节点的next域赋初值NULL
...
int headInsert(Node *List,elemType e)    //传入链表和要插入的值 
{
	...
	p->prev=List;                         //新节点p指向首节点 
	p->next=List->next;                   //把原来第一个节点的地址给新节点p，即p指向原第一节点
	if(List->next!=NULL)
	{
		List->next->prev=p;               //当链表原来有节点时，将原第一节点的prev指向新节点p        	
	}
	List->next=p;                         //把新节点p的地址给首节点，即首节点指向新节点p 
}
...
int insertNode(Node *List,int pos,elemType e)
{
	...
	q->data=e;                           //把值赋给新的节点 
	q->prev=p;                           //使新节点q指向前一个节点 
	q->next=p->next;                     //把新节点q前一个节点所存的地址给q，即q指向后一个节点 
	if(p->next!=NULL)
	{
		p->next->prev=q;                 //当原来p节点指向不为NULL时，使该节点的prev指向新节点q 
	}
...
int deleteNode(Node *List,int pos)
{
	...
	else
	{
		...
		if(q->next!=NULL)
		{
			q->next->prev=p;             //当原来q节点指向不为NULL时，使该节点的prev指向q的前驱节点p 
		}
